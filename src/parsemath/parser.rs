...
impl<'a> Parser<'a> {
  ...
  fn generate_ast(&mut self, oper_prec: OperPrec) -> Result<Node, ParseError> {
    // processes numeric tokens, negative number tokens, and expressions in parentheses
    let mut left_expr = self.parse_number()?;

    // the expression containing an operator with higher precedence is executed before 
    // an expression containing an operator with lower precedence  
  ...
  // Construct Operator AST nodes by checking whether the token is Add, Subtract, Multiply, Divide, or Caret
